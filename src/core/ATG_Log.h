...
namespace atg
{
  ...
  public:
    ...
    static void w(const __FlashStringHelper *tag, const String &msg)
    {
      out(LogLevel::Error, tag, msg);
    }

    static void w(const __FlashStringHelper *tag, const char *msg)
    {
      out(LogLevel::Error, tag, String(msg));
    }
  ...
  private:
    static void out(LogLevel lvl, const __FlashStringHelper *tag, const String &msg)
    {
      ...
      (void)msg;
#else
      if ((uint8_t)lvl > (uint8_t)_level)
        return;
      if (_sink)
        _sink->write(lvl, tag, msg);
#endif
    }
